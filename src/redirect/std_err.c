...
static int get_fd_err_1(run_t *run, int i, int finded)
{
...
static int get_fd_err_2(run_t *run, int i, int finded)
{
    int fd = STDERR_FILENO;
...
int get_fd_err(run_t *run)
{
    ...
    for (int i = 0; run->command[i] != NULL && fd >= 0; i++) {
        if (my_strcmp(run->command[i], "2>") == 0 ||
            my_strcmp(run->command[i], "&>") == 0) {
            finded++;
            fd = get_fd_err_1(run, i, finded);
        }
        if (my_strcmp(run->command[i], "2>>") == 0 ||
            my_strcmp(run->command[i], "&>>") == 0) {
            finded++;
            fd = get_fd_err_2(run, i, finded);
        }
    }
    if (finded <= 1 && fd >= 0)
        return fd;
